...
#=====================================================================================================
#MIX-UP FUNCTION
def mixupImage(imgBatch, label, alpha=0.2):
    if alpha > 0:
        #Get lambda value from beta distribution with alpha parameter; bigger alpha = more mixing
        lam = np.random.beta(alpha, alpha) #Same alpha value for both classes = no skewing
    else:
        lam = 1 #Mix-up is disabled if alpha<=0

    batchSize = imgBatch.size()[0] #1st dimension tells batch size
    index = torch.randperm(batchSize) #Randomize number (image index) order from 1 to batchSize; output = 1D-tensor

    mixedImg = lam*imgBatch + (1-lam)*imgBatch[index, :] #New image =  Original & shuffled image weighted sum
    originalLabel, shuffledLabel = label, label[index]
    
    return mixedImg, originalLabel, shuffledLabel, lam

def mixupCriterion(criterion, pred, originalLabel, shuffledLabel, lam):
    loss = lam*criterion(pred, originalLabel) + (1-lam)*criterion(pred, shuffledLabel)
    #Compute loss for mixup via criterion function
    #Mix losses instead of labels; mathematically equivalent to mixing labels
    return loss
#=====================================================================================================
...
#Change path name depending on alpha value to prevent overwritting of best models
pthName = "best_resnet50_4(alpha0.05).pth" #For when alpha=0.05
#pthName = "best_resnet50_4(alpha0.1).pth" #For when alpha=0.1
#pthName = "best_resnet50_4(alpha0.2).pth" #For when alpha=0.2
#=====================================================================================================
...
#=====================================================================================================
#CREATE MODEL, CLASS WEIGHTS TO HANDLE CLASS IMBALANCES, OPTIMIZER, LEARNING RATE SCHEDULER, ETC.
...
for epoch in range(epochs):
    ...
    for images, labels in trainLoader:
        ...
        mixedImg, originalLabel, shuffledLabel, lam = mixupImage(images, labels, alpha)

        optimizer.zero_grad() #Zero out old gradients
        outputs = model(mixedImg)
        
        loss = mixupCriterion(criterion, outputs, originalLabel, shuffledLabel, lam)
        loss.backward()
        optimizer.step()
